...
void test_vm_macro_hygiene(void) {
    VM vm;
    vm_init(&vm);
    vm_register_primitives(&vm);
    global_vm_ptr = &vm;
...
void test_vm_letrec_syntax(void) {
    VM vm;
    vm_init(&vm);
    vm_register_primitives(&vm);
...
int main(void) {
    ...
    RUN_TEST(test_vm_macro);
    RUN_TEST(test_vm_local_macro);
    RUN_TEST(test_vm_macro_hygiene);
    RUN_TEST(test_vm_letrec_syntax);
    return UNITY_END();
}
